...
```sh
JAVA_HOME_21=/path/to/jdk21 rtk tool/scripts/package/package_server_jar.sh
```
...
脚本里的 OrzRepacker 路径需要按自己的本机目录配置，例如：

```text
/path/to/OrzRepacker-v1.157.48
```

如果你自己的 OrzRepacker 放在别的位置，可以设置：

```sh
ORZ_REPACKER_HOME=/你的/OrzRepacker路径 rtk tool/scripts/package/pack_img_wz.sh ...
```
...
```text
tool/scripts/audit/            资源和链路审计
tool/scripts/migration/        一次性内容迁移
tool/scripts/package/          服务端与客户端 WZ 打包
tool/scripts/runtime/          本地服务启动
tool/scripts/wz/               WZ/IMG 解析包装
tool/scripts/png2canvas/       PNG 写入 IMG Canvas 网页工具
tool/scripts/patch-client/     客户端 EXE 补丁
tool/scripts/patch-skill/      技能数据、特效与行为补丁
tool/scripts/patch-boss/       Boss 兼容补丁
tool/scripts/patch-equipment/  装备数据补丁
```
...
rtk tool/scripts/package/pack_img_wz.sh --input clien/Data/Skill --output "$HOME/Downloads/Skill.wz" --version 83
```

脚本默认使用 `-Xms512m -Xmx8g -XX:+UseG1GC` 运行打包进程。打包 `Character` 或“全部目录”仍出现 `java.lang.OutOfMemoryError: Java heap space` 时，可以调大堆内存：

```sh
PACK_IMG_WZ_JAVA_OPTS="-Xms512m -Xmx12g -XX:+UseG1GC" rtk tool/scripts/package/pack_img_wz.sh --input clien/Data/Character --output "$HOME/Downloads/Character.wz" --version 83
```
...
```sh
rtk tool/scripts/runtime/start_server.sh --background
```
