...
story、easy 等模式选择不同 Mob。当前兼容战斗保留 `8880140 -> 8880141`，并将
硬模式狂暴阶段修正为 TMS 指定的 `8880152`；成功和失败过渡分别使用
`8880153/8880154`。旧的 `8880142.img` 保持原始字节不变，但不再进入战斗链。
项目仍不具备现代 Boss Party 或 `fieldType 147/152` 运行时。
...
- `450004250`：远征实例 P2/P3，生成 `8880141`，其 revive 以记录级增量修改为
  `8880152`；击败狂暴本体后 revive `8880153` 并延迟 5 秒结算。
...
- `Butterfly`：按 Boss HP 使用 `5000/4500/4000/3000/2000ms` 周期和
  `5/7/10/15/20` 生成量，保留 P1/P2 各 40 个坐标；计数严格封顶 40 时触发爆发，
  不再把每轮生成量截断为 4 或把可见蝴蝶截断为 12。TMS P1 第 40 个
  坐标为明显越界的 `17100`，兼容投影修正为同一列的 `1710`。爆发视频不再
  误用静止的 `Butterfly/butterfly/0/fly+bomb`，而是让 40 个位置按九种
  `butterflies/0..8/fly_phase2` 变体在 540ms 内向露希妲汇聚，随后依次播放
  `change`、`prepare` 环形眼和 `erase` 上升消散；回收动画开始时同步清理实际
  蝴蝶 Mob，并在 1800ms 的 `prepare` 转折点结算伤害。
...
- `Fury`：P2 的 `8880141` revive 到 TMS 硬模式狂暴本体 `8880152`，进入时播放
  45 秒雾化倒计时并停止所有 P2 周期技能。45 秒内击败会生成成功过渡
  `8880153`，立即停止雾化视频并在 5 秒后结算；超时则无击杀移除狂暴本体，
  生成失败过渡 `8880154`，3 秒时结算全图 100% HP 伤害，10 秒后移除失败过渡，
  在 `(711,-786)` 重新生成 16% HP 的 `8880141`。阶段转换、结算、超时和空队伍
  都会取消旧任务并清理支援及过渡 Mob。

## MCV 场景链

复杂场景由 `Map/Effect.img/customSkill/lucid` 的十九个 `7x5` ARGB4444 标记
...
签名，将标记代码 15–32 路由到上述视频，并将代码 33 作为
`furyStopVideoLayer` 调用 `BDV_StopChannel(BDV_CHANNEL_BOSS_SCENE)`；既不与普通
视频技能代码 0 冲突，也不与卡琳代码 1–14 冲突。
