...
- 阿卡伊勒死亡后如果回场再进 Boss 图发现 Boss 没了，问题通常不在怪物资源，而在事件地图范围和脚本加载目录。`272020200` 的 `returnMap` 是 `272020400`、`forcedReturn` 才是 `272020110`；死亡确认会走 `returnMap=272020400`，玩家再从 `272020400/west00` 回 `272020200`。如果 `ArkariumBattle.changedMap()` 只把 `272020200` 当事件地图，玩家死亡到 `272020400` 时会被 `unregisterPlayer()`，回 Boss 图就不再是同一个 EIM，所以 Boss 消失或像已结算。最终兼容语义：`playerRevive()` 返回 `true`，让服务端标准复活进入 `272020400`；`getEventMaps()/isEventMap()` 同时包含 `272020110`、`272020200`、`272020400`；入口 portal/NPC 如果发现玩家仍挂着未结算的 `ArkariumBattle`，直接送回同一个 `272020200` 实例图继续打 Boss；玩家离开这三张图、主动退出或断线时才注销并在空场时 `dispose()`。当前配置 `gms.service.language=zh-CN` 会优先加载 `scripts-zh-CN`，所以同类修复必须同步到 `scripts-zh-CN/event/ArkariumBattle.js`、`scripts-zh-CN/portal/portalNPC1.js` 和中文 NPC 入口脚本；只改 `scripts/` 对实机不生效。若线上已经被旧脚本卡住，需要重载/重启服务端或等旧锁超时，新脚本只能防止后续再次留下异常 EIM。
