...
- 全蚀之力 MCV：完整男性主轨43帧加终结轨44帧，共87帧、5220ms、`10902999`字节；已移除原先把前43帧按每6帧抽样的策略；
...
银河星爆、全蚀之力、灵魂蚀日和宇宙之花在动画期间持续发送近战攻击包。每个后续攻击包先直接发送给施法者本人，再以排除施法者的方式广播给地图其他玩家；由于旧端本地角色不会稳定显示服务器重放的自身近战飘字，四个技能均使用通用 `INDEXED` 模板，并独立结算一次合计真实伤害。默认按原生 `120ms * hitIndex` 为施法者逐段补发数字；银河星爆因 TMS multi-attack 本身为60ms密集节奏，单独使用60ms，避免180ms replay之间叠加最多6套1680ms数字序列。银河星爆和全蚀之力的攻击时间点来自 TMS `multiAttackInfo`，将小于180ms的密集点节流后保留各爆发段；灵魂蚀日按600ms周期攻击至19800ms；宇宙之花按原始 `time=15000ms`、`subTime=450ms` 执行33段攻击，最后一段为14850ms。

第一次延迟命中仍走完整 `applyAttack`，只扣一次MP并应用一次技能效果。银河星爆、全蚀之力和灵魂蚀日的后续命中优先复用首次攻击包的非空伤害值，并在每个时间点重新扫描施法时固定的技能矩形。宇宙之花则复用海龙螺旋的追踪攻击机制，每450ms按角色当前位置重新计算技能矩形。四个技能在空目标施放时都不提前生成伤害：无目标周期直接跳过，后续周期检测到目标后才按角色最大基础伤害和 `11100000` 剑术精通的实际最小伤害范围生成本周期的逐段模板，不把一个最大值复制成固定伤害，也不跨周期缓存兜底模板。所有持续攻击均从范围内当前存活的怪物中选取最多 `mobCount` 个目标；角色死亡、离线或换图后停止后续任务。
...
6. 验证银河星爆119帧、全蚀之力87帧和灵魂蚀日159帧连续播放；全蚀之力应保持87帧均为60ms，灵魂蚀日应持续20秒。
