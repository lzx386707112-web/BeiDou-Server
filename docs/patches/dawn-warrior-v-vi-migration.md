...
第一次延迟命中仍走完整 `applyAttack`，只扣一次MP并应用一次技能效果。银河星爆、全蚀之力和灵魂蚀日的后续命中只复用首次攻击包的非空真实伤害值，并在每个时间点重新扫描施法时固定的技能矩形。宇宙之花每450ms按角色当前位置重新计算技能矩形。四个技能的时间轴在首次空目标时仍继续运行，但没有客户端真实伤害模板的周期只跳过，不创建攻击包、飘字、仇恨或掉血，也不再按角色面板估算约 `500000` 的兜底单段。客户端后续发送的新攻击包仍由标准攻击入口独立解析。所有有效的持续攻击均从范围内当前存活的怪物中选取最多 `mobCount` 个目标；角色死亡、离线或换图后停止后续任务。
