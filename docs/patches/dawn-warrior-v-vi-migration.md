...
第一次延迟命中仍走完整 `applyAttack`，只扣一次MP并应用一次技能效果；后续命中复用首次攻击包的一组伤害值，但像毒雾一样在每个时间点重新扫描施法时固定的技能矩形，从范围内当前存活的怪物中选取最多 `mobCount` 个目标，不再绑定首次攻击的 OID。角色死亡、离线或换图后，对应后续任务会停止，不会把旧地图 OID 带到新地图。服务端以 `DW_ANIM v3` 记录调度、第二段和末段的实际目标数，用于区分定时器未执行与范围内无存活目标。
