...
当前范围已按 `2121006/effect` 动画尺寸调整。最初只缩小技能图片到 80% 时，图片尺寸变小了，
但 `origin.x` 仍保持原值，导致技能画面右边缘离角色多出一段距离。处理方式是只把
`2121006/effect` 21 帧的 `origin.x` 按 80% 重算，`origin.y` 保持不变，避免上下位置也被带偏。

调整前解析客户端 `clien/Data/Skill/212.img`，`2121006/effect` 21 帧的联合画面边界约为：
...
为了让“技能范围尽量等于技能画面大小”，并照顾小怪脚底/碰撞框偏低的问题，实际写入：
...
- 数据补丁脚本：`tool/scripts/patch-skill/patch_2121006_aoe.py`
- exe 补丁脚本：`tool/scripts/patch-client/patch_2121006_exe_aoe.js`
...
```bash
rtk python3 tool/scripts/patch-skill/patch_2121006_aoe.py --dry-run
rtk python3 tool/scripts/patch-skill/patch_2121006_aoe.py
```

exe 层：

```bash
rtk node tool/scripts/patch-client/patch_2121006_exe_aoe.js --dry-run
rtk node tool/scripts/patch-client/patch_2121006_exe_aoe.js
```
...
```bash
rtk tool/scripts/wz/wzpy.sh convert clien/Data/Skill/212.img --region GMS -o <临时目录>/212.after.json
```
