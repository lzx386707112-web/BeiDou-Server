...
空目标施放不代表发生了一次伤害。魂骑士四个持续技能会保留完整时间轴，但只有首次攻击包捕获到非空客户端伤害模板时才执行后续重放；没有真实模板的周期直接跳过，不创建攻击包、飘字、仇恨、掉血或服务端估算的兜底伤害。客户端后续发送的新攻击包由标准攻击入口重新解析，不能用同一个不可更新的 `AttackInfo` 或约 `500000` 的估算单段冒充后续真实伤害。
