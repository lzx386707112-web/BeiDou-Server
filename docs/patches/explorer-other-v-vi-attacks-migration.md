...
本迁移以 TMS v280 为源，将旧端已有完整职业体系的 9 个冒险家职业五、六转攻击技能迁移到
...
通过 SP 单独加点。该脚本只识别 12 个冒险家四转职业，不改变骑士团的自动授予逻辑。
...
兼容 DLL v41 将 9 个职业的全部可施放入口和隐藏回放阶段分别送入旧端近战、魔法或远程攻击
...
`ExplorerOtherSkillCompat` 按 TMS `multiAttackInfo` 的累计 `attackTime` 展开 20 个可施放技能的
...
18 个带 TMS `screen/video` 的可施放技能导出为 `1280x720` MCV。每条源视频轨按自身 Alpha 可见
...
| 冰雷 | `2221027` / `explorer-2221027.mcv` | 92 | 5520ms |
| 冰雷 | `2221030` / `explorer-2221030.mcv` | 42 | 2520ms |
...
契约测试核对 9 个主动技能数组、服务端 30 级参数、客户端真实 hit、18 个 MCV 的尺寸/帧数/Alpha、
DLL 映射、Map Effect 标记，以及 20 个 TMS 多阶段时间轴。最终仍需在 Windows/Winlator 客户端
