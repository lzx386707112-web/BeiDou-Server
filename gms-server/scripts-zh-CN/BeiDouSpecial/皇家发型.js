...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        cm.sendSimple(buildStyleMenu("请选择想更换的发型款式。", bases, "发型"));
    } else if (status == 1) {
        var bases = getGenderBases();
        ...
        cm.sendSimple(buildStyleMenu("请选择想更换的发色。", colorOptions, "发色"));
    } else if (status == 2) {
...
function buildStyleMenu(title, styles, label) {
    var text = title + "\r\n\r\n";
    for (var i = 0; i < styles.length; i++) {
        text += "#L" + i + "#" + label + " " + (i + 1) + "  #b" + styles[i] + "#k#l\r\n";
    }
    return text;
}
