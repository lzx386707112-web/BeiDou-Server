...
function setup(channel) {
    var eim = em.newInstance("VONBON" + channel);
    ...
    // 自动召唤 VONBON BOSS (8910100)
    var mob = LifeFactory.getMonster(8910100);
    battleMap.spawnMonsterOnGroundBelow(mob, new java.awt.Point(489, 454));
...
function clearPQ(eim) {
    ...
    eim.dropMessage(5, "[远征队] 恭喜！你们成功击败了 半半！");
    updateGateState(0);
...
function isVONBON(mob) {
    var mobid = mob.getId();
    return (mobid == 8910100);
}

function monsterKilled(mob, eim) {
    if (isVONBON(mob) && eim.getIntProperty("defeatedBoss") == 0) {
