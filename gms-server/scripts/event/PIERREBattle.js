 ...
 * @modified: 对齐 TMS 201/40 分裂与 linkHP 双形态机制
 */
...
var bossId = 8900000;         // 皮埃尔初始形态
var treasureMobId = 8900003;  // TMS 宝箱怪物
...
function isPierre(mob) {
    return mob.getId() >= 8900000 && mob.getId() <= 8900002;
}

function hasAlivePierre(map) {
    for (var id = 8900000; id <= 8900002; id++) {
...
// 初始形态分裂时先生成红蓝形态，最后一个存活形态死亡才通关。
function monsterKilled(mob, eim) {
    if (isPierre(mob) && !hasAlivePierre(mob.getMap()) && eim.getIntProperty("defeatedBoss") == 0) {
