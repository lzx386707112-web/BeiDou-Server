...
if ! grep -qF "extended equipment body-part validation" "$source_file"; then
  echo "refusing to overwrite DawnWarriorSkillCompat.dll: that filename is the HP/MP wrapper, and this source is missing the verified extended-equipment compatibility hooks" >&2
  exit 1
fi
