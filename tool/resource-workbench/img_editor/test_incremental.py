...
class IncrementalMutationTests(unittest.TestCase):
    ...
    def test_replace_record_preserves_sibling_bytes(self):
        original = _sample_img()
        before_tail = _record(original, ["tail"])
        tail_bytes = original[before_tail.start:before_tail.end]
        replacement = WzSubProperty("info")
        replacement.add(WzIntProperty("level", 25, replacement))
        replacement.add(WzStringProperty("name", "replacement value", replacement))

        changed = replace_img_record(original, ["info"], replacement, region="GMS").data
        after_tail = _record(changed, ["tail"])
        self.assertEqual(tail_bytes, changed[after_tail.start:after_tail.end])
        self.assertEqual(_record(changed, ["info"]).name, "info")
