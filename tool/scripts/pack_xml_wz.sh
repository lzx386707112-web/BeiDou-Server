...
usage() {
  cat <<'USAGE'
用法:
  rtk tool/scripts/pack_xml_wz.sh
  rtk tool/scripts/pack_xml_wz.sh --input gms-server/wz/Skill.wz --output /tmp/Skill.wz --version 83
  rtk tool/scripts/pack_xml_wz.sh -i gms-server/wz/Skill.wz -o /tmp/Skill.wz
...
未能自动找到 JDK 21。
请重新执行:
  JAVA_HOME_21=/path/to/jdk21 rtk tool/scripts/pack_xml_wz.sh ...
EOF
  exit 2
fi
