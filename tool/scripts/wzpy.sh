...
usage() {
  cat <<'USAGE'
Usage:
  tool/scripts/wzpy.sh convert clien/Data/Skill/100.img --region GMS -o /tmp/100.json
  tool/scripts/wzpy.sh convert clien/Data/Skill/100.img --auto-region
  tool/scripts/wzpy.sh ui
  tool/scripts/wzpy.sh ui clien/Data/Skill/Skill.wz
  tool/scripts/wzpy.sh ui clien/Data/Skill --region EMS
  tool/scripts/wzpy.sh python -c 'import wzpy; print(wzpy.WzImage)'
...
Use convert instead:
  tool/scripts/wzpy.sh convert "$ui_path" --region GMS -o /tmp/$(basename "$ui_path").json
EOF
        exit 2
      fi
...
This repo's client data is extracted as standalone .img files. Use convert on a single .img:
  tool/scripts/wzpy.sh convert "$ui_path/100.img" --region GMS -o /tmp/100.img.json
