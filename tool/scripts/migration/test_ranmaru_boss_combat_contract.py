...
def expect_skill_levels(errors: list[str]) -> None:
    ...
    for skill_id, level in ((100, 25), (145, 19), (128, 23), (133, 18), (176, 6)):
        node = xml.find(f'./imgdir[@name="{skill_id}"]/imgdir[@name="level"]/imgdir[@name="{level}"]')
        if node is None:
            errors.append(f"missing GMS MobSkill {skill_id}/{level}")
...
def main() -> int:
    ...
    if "applyRanmaruScreenCrack" not in java or "isMoriRanmaruHard" not in java:
        errors.append("MobSkill.java missing Ranmaru 176 projection")
