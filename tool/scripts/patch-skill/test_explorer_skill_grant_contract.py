...
class ExplorerSkillGrantContractTest(unittest.TestCase):
    ...
    def test_fifth_job_goddess_grants_item_to_supported_explorers(self):
        ...
        self.assertIn("ADVANCEMENT_CASH = 10000", goddess)
        self.assertIn("buildExplorerFifthJobPrompt()", goddess)
        self.assertIn("unlockExplorerFifthJob()", goddess)
        status_zero = goddess[goddess.index("if (status === 0)"):goddess.index("if (status === 1)")]
        self.assertIn("cm.sendSimple(menu)", status_zero)
        self.assertNotIn("prepareExplorerFifthJob()", status_zero)
        self.assertIn('isExplorer ? "完成冒险家五转" : "完成骑士团四转"', status_zero)
        self.assertIn("合成英雄币", status_zero)
        self.assertNotIn("我是富豪我想走捷径", goddess)
        self.assertNotIn("exchangeCoreGemstones", goddess)
        self.assertNotIn("SHORTCUT_CORE_GEMSTONE_COUNT", goddess)
        self.assertIn("getMissingAdvancementRequirements()", goddess)
        self.assertIn("deductAdvancementCosts()", goddess)
        self.assertEqual(2, goddess.count("var missing = getMissingAdvancementRequirements();"))
        self.assertEqual(2, goddess.count("deductAdvancementCosts();"))
        self.assertIn("cm.getPlayer().getCashShop().getCash(1) < ADVANCEMENT_CASH", goddess)
        self.assertIn("cm.getPlayer().getCashShop().gainCash(1, -ADVANCEMENT_CASH)", goddess)
        self.assertIn("cm.gainItem(HERO_COIN_ID, -1)", goddess)
        self.assertIn("cm.gainItem(CORE_GEMSTONE_ID, -CORE_GEMSTONE_COUNT)", goddess)
        self.assertIn("cm.gainMeso(-ADVANCEMENT_MESO)", goddess)
