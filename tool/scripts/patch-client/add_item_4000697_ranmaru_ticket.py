#!/usr/bin/env python3
"""Incrementally install custom Etc item 4000697 (森兰丸挑战门票)."""
...
PREVIOUS_CUSTOM = "04000696"
PREVIOUS_STRING = "4000696"
ITEM_NAME = "森兰丸挑战门票"
ITEM_DESC = "可进入森兰丸地盘的免费门票，使用1次即作废。"
SOURCE_ICON = Path(__file__).resolve().parent / "assets/4000697.png"
...
def validate_item(item_data: bytes) -> None:
    ...
    if PREVIOUS_CUSTOM not in names:
        raise RuntimeError("previous custom Etc 4000696 is missing")
    if names.index(ITEM_NODE) != names.index(PREVIOUS_CUSTOM) + 1:
        raise RuntimeError("4000697 is not immediately after 4000696")
    if names[names.index(ITEM_NODE) + 1] != ITEM_ANCHOR:
        raise RuntimeError("4000697 is not immediately before 04000828")
...
def validate_string(string_data: bytes) -> None:
    ...
    if names.index(STRING_NODE) != names.index(PREVIOUS_STRING) + 1:
        raise RuntimeError("string 4000697 is not immediately after 4000696")
    if names[names.index(STRING_NODE) + 1] != STRING_ANCHOR:
        raise RuntimeError("string 4000697 is not immediately before 4000828")
...
def validate_xml(item_xml: str, string_xmls: dict[Path, str]) -> None:
    ...
    if item_names.index(ITEM_NODE) != item_names.index(PREVIOUS_CUSTOM) + 1:
        raise RuntimeError("server Item XML order mismatch for 4000697")
...
def main() -> int:
    ...
    second = apply(build())
    if first != second:
        raise RuntimeError(f"item 4000697 insert is not idempotent: {first} vs {second}")
    print("item 4000697 森兰丸挑战门票 ok")
