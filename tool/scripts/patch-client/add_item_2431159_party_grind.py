#!/usr/bin/env python3
"""Incrementally install consume item 2431159 (基友集合)."""
...
PREVIOUS_ITEM = "02431158"
PREVIOUS_STRING = "2431158"
ITEM_NAME = "基友集合"
ITEM_DESC = "发出一道命令，马上召唤好基友陪你搬砖，搬砖时间半小时。"
SOURCE_ICON = Path(__file__).resolve().parent / "assets/2431159.png"
...
def make_item_node(pixels: Image.Image) -> WzSubProperty:
    ...
    spec.add(WzStringProperty("script", "consume_2431159", spec))
    return item
...
def validate_item(item_data: bytes) -> None:
    ...
    if PREVIOUS_ITEM not in names:
        raise RuntimeError("previous consume 2431158 is missing")
    if names.index(ITEM_NODE) != names.index(PREVIOUS_ITEM) + 1:
        raise RuntimeError("2431159 is not immediately after 2431158")
    if names[names.index(ITEM_NODE) + 1] != ITEM_ANCHOR:
        raise RuntimeError("2431159 is not immediately before 02436037")
    ...
    if str(arc.child_value(spec, "script")) != "consume_2431159":
...
def validate_string(string_data: bytes) -> None:
    ...
    if names.index(STRING_NODE) != names.index(PREVIOUS_STRING) + 1:
        raise RuntimeError("string 2431159 is not immediately after 2431158")
    if names[names.index(STRING_NODE) + 1] != STRING_ANCHOR:
        raise RuntimeError("string 2431159 is not immediately before 2029006")
...
def validate_xml(item_xml: str, string_xmls: dict[Path, str]) -> None:
    ...
    if item_names.index(ITEM_NODE) != item_names.index(PREVIOUS_ITEM) + 1:
        raise RuntimeError("server Item XML order mismatch for 2431159")
    ...
    if spec_values.get("script") != "consume_2431159":
...
def main() -> int:
    ...
    second = apply(build())
    if first != second:
        raise RuntimeError(f"item 2431159 insert is not idempotent: {first} vs {second}")
    print("item 2431159 基友集合 ok")
