...
run_interactive() {
  ...
  case "$target" in
    "全部目录")
      ...
      if [[ ${#base_imgs[@]} -gt 0 || ${#inputs[@]} -gt 0 ]]; then
        ...
        printf '#   %s\n' "${base_imgs[@]}"
        interactive_quote_cmd "$ROOT/tool/scripts/package/pack_img_wz.sh" --input "<base-temp-dir>" --output "$output_dir/Base.wz" --version "$version"
      fi
      for input in "${inputs[@]}"; do
        base="$(basename "$input")"
        interactive_quote_cmd "$ROOT/tool/scripts/package/pack_img_wz.sh" --input "$input" --output "$output_dir/${base}.wz" --version "$version"
      done
      echo
      ...
      if interactive_ask_yes_no "现在执行" "y"; then
        ...
        if [[ ${#base_imgs[@]} -gt 0 || ${#inputs[@]} -gt 0 ]]; then
          ...
          "$ROOT/tool/scripts/package/pack_img_wz.sh" --input "$base_tmp_dir" --output "$output_dir/Base.wz" --version "$version"
        fi
        ...
        for input in "${inputs[@]}"; do
          ...
          "$ROOT/tool/scripts/package/pack_img_wz.sh" --input "$input" --output "$output" --version "$version"
        done
        echo
        echo "全部打包完成。"
        return 0
  ...
  echo
  echo "即将执行命令:"
  interactive_quote_cmd "$ROOT/tool/scripts/package/pack_img_wz.sh" "${args[@]}"
  echo

  if interactive_ask_yes_no "现在执行" "y"; then
    exec "$ROOT/tool/scripts/package/pack_img_wz.sh" "${args[@]}"
  fi
...
未能自动找到 JDK 21。
请重新执行:
  JAVA_HOME_21=/path/to/jdk21 rtk tool/scripts/package/pack_img_wz.sh ...
EOF
  exit 2
fi
