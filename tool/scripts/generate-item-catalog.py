#!/usr/bin/env python3
"""
生成物品预览目录：从 Item.wz 提取图标，生成图集和 catalog.json
参考 generate-equipment-catalog.py 的实现
"""
...
def extract_icon_from_xml(img_xml_path, item_id):
    """从XML中提取图标数据"""
    try:
        ...
        # 查找目标物品节点
        item_node = root.find(f".//imgdir[@name='{item_id}']")
        ...
        # 获取图标尺寸
        width = int(icon_node.get("width", 0))
        height = int(icon_node.get("height", 0))
        
        if width <= 0 or height <= 0:
            return None
        
        # 查找实际的图像数据（在子目录中）
        # 注意：这里需要实际的WZ解析库来获取像素数据
        # 暂时返回尺寸信息，实际实现需要wzpy
        return {"width": width, "height": height, "node": icon_node}
    except Exception as e:
        print(f"Warning: Failed to extract icon from {img_xml_path}: {e}")
        return None
...
def create_atlas(items_with_icons, cell_size=CELL_SIZE):
    """创建图集（需要wzpy来解码canvas）"""
    # 这里需要实际的wzpy库来解码canvas数据
    # 暂时返回空结果
    print("Note: Atlas generation requires wzpy library for canvas decoding")
    return {}, {}
...
def main():
    ...
    # 生成catalog.json（不含图集信息，因为需要wzpy解码）
    catalog = {
        "version": 1,
        "cellSize": CELL_SIZE,
    ...
    # 注意：实际的图集生成需要使用wzpy库解码canvas
    # 这里只生成catalog.json的框架
    print("\nNote: To generate icon atlases, use wzpy library to:")
    print("1. Decode canvas data from Item.wz")
    print("2. Create atlas images for each category")
    print("3. Update catalog.json with x,y coordinates")
